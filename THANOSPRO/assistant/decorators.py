...
def THANOSPRO_cmd(add_cmd, is_args=False):
    def cmd(func):
        THANOSPRO = bot.tgbot
        if is_args:
...
def is_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            THANOSPRO = bot.tgbot
            perms = await THANOSPRO.get_permissions(event.chat_id, event.sender_id)
...
def is_bot_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            THANOSPRO = bot.tgbot
            boat = await THANOSPRO.get_me()
            perms = await THANOSPRO.get_permissions(event.chat_id, boat)
            if perms.is_admin:
                await func(event)
            else:
