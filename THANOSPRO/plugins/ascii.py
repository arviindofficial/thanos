...
async def _(event):
    ...
    async with event.client.conversation(bot) as conv:
        try:
            ...
            return
    await event.client.send_file(event.chat_id, file=output_op, caption=f"ASCII art By :- {rishu_mention}", force_document=False)
    await kraken.delete()  
    await event.client.delete_messages(
        conv.chat_id, [first.id, response.id, second.id, output_op.id]
    )

@rishu_cmd(pattern="line(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.reply_to_msg_id:
    ...
    async with event.client.conversation(bot) as conv:
        try:
            ...
            return
    await event.client.send_file(event.chat_id, file=output_op, caption=f"Lines By :- {rishu_mention}", force_document=False)
    await kraken.delete()
    await event.client.delete_messages(
        conv.chat_id, [first.id, response.id, second.id, output_op.id]
    )
