...
async def all_groups_id(rishu):
    rishugroups = []
    async for dialog in rishu.client.iter_dialogs():
        entity = dialog.entity
        if isinstance(entity, Channel) and entity.megagroup:
            rishugroups.append(entity.id)
    return rishugroups


@rishu_cmd(pattern="frwd$")
async def _(event):
