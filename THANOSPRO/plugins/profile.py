...
async def _(event):
    ...
    if user.first_name.startswith(OFFLINE_TAG):
        ...
        return
    rishu = await eor(event, "**Changing Profile to Offline...**")
    ...
    if photo:
        ...
        try:
            ...
            await eod(rishu, "**Changed profile to OffLine.**")
    try:
    ...
    try:
        ...
        await eod(rishu, result)
    except Exception as e:
        await eod(rishu, str(e))


@rishu_cmd(pattern="online$")
async def _(event):
    user_it = "me"
    user = await event.client.get_entity(user_it)
    if user.first_name.startswith(OFFLINE_TAG):
    ...
    if photo:
        ...
        try:
            ...
            await eod(rishu, "**Changed profile to Online.**")
    try:
        os.system("rm -fr donottouch.jpg")
    except Exception as e:
        LOGS.warn(str(e))
...
@rishu_cmd(pattern="ppic$")
async def _(event):
    reply_message = await event.get_reply_message()
    rishu = await eor(event, "Downloading Profile Picture to my local ...")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    photo = None
    try:
        ...
        if photo:
            await rishu.edit("Uploading profile picture...")
            file = await event.client.upload_file(photo)
            try:
                await event.client(functions.photos.UploadProfilePhotoRequest(file))
            except Exception as e:
...
@rishu_cmd(pattern="username(?:\s|$)([\s\S]*)")
...
async def count(event):
    ...
    await rishu.edit(result, parse_mode="HTML")


@rishu_cmd(pattern="delpfp$")
...
CmdHelp("profile").add_command(
  ...
  "pbio", "<text>", "Changes your Telegram bio", ".pbio rishuo there, This iz my bio"
).add_command(
