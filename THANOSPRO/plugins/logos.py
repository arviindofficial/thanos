...
@rishu_cmd(pattern="logo(?:\s|$)([\s\S]*)")
async def _(event):
    rishu = await eor(event, "`Processing.....`")
    text = event.text[6:]
    if text == "":
        await eod(rishu, "**Give some text to make a logo !!**")
        return
    cid = await client_id(event)
    rishu_mention = cid[2]
    start = datetime.datetime.now()
    fnt = await get_font_file(event.client, "@thanosfonts")
    if event.reply_to_msg_id:
        ...
        await rishu.edit("Picked a Logo BG...")
        async for i in event.client.iter_messages("@thanoslogo", filter=InputMessagesFilterPhotos):
            PICS_STR.append(i)
    ...
    await event.client.send_file(
        event.chat_id,
        file_name,
        caption=f"**Made By :** {rishu_mention} \n**Time Taken :** `{ms} seconds`",
    )
    await rishu.delete()
    try:
        os.remove(file_name)
