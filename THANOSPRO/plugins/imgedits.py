...
if not os.path.isdir("./THANOSPRO/"):
    os.makedirs("./THANOSPRO/")


@rishu_cmd(pattern="feye$")
async def fun(event):
    path = "omk"
    rishu = await eor(event, "Editing In Progress...")
...
@rishu_cmd(pattern="warp$")
async def fun(event):
    path = "omk"
    rishu = await eor(event, "Warping In Progress...")
    reply = await event.get_reply_message()
    lol = await event.client.download_media(reply.media, path)
...
@rishu_cmd(pattern="distort$")
async def fun(event):
    path = "omk"
    rishu = await eor(event, "Distortion In Progress...")
    reply = await event.get_reply_message()
    lol = await event.client.download_media(reply.media, path)
    file_name = "dist.jpg"
    hehe = path + "/" + file_name
