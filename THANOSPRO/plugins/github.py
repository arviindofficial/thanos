...
@rishu_cmd(pattern="commit(?:\s|$)([\s\S]*)")
async def download(event):
    ...
    THANOSPRO = await eor(event, "Processing ...")
    ...
    try:
        ...
        await eod(THANOSPRO, str(e))
    else:
        end = datetime.datetime.now()
        ms = (end - start).seconds
        await event.delete()
        await THANOSPRO.edit(
            "Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
        )
        await THANOSPRO.edit("Committing to Github....")
        await git_commit(downloaded_file_name, path, branch, THANOSPRO)


async def git_commit(file_name, path, branch, THANOSPRO):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await THANOSPRO.edit("`File Already Exists`")
            create_file = False
    path = path
    file_name = file_name
    if create_file == True:
        ...
        try:
            repo.create_file(
                path, f"Uploaded file {file_name} by Շђคภ๏ร-קг๏", commit_data, branch=branch
            )
            ...
            await eod(THANOSPRO, "Cannot Upload File")
    else:
        return await eod(THANOSPRO, "`Committed Suicide`")


@rishu_cmd(pattern="github(?:\s|$)([\s\S]*)")
async def _(event):
...
CmdHelp("github").add_command(
  "commit", "<reply to a file> <path>|<branch>", "Uploads the file on github repo as provided in Heroku Config GIT_REPO_NAME. In short makes a commit to git repo from Userbot", "commit ./THANOSPRO/plugins/example.py|master"
).add_command(
