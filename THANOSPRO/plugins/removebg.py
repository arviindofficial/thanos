...
async def _(event):
    ...
    if event.reply_to_msg_id and input_str == "":
        ...
        rishu = await eor(event, "`Analysing...`")
        ...
        try:
            ...
            await rishu.edit("`Removing background of this media`")
            file_name = toimage(file_name)
            response = ReTrieveFile(file_name)
            os.remove(file_name)
    elif input_str:
        rishu = await eor(event, "`Removing Background of this media`")
    ...
    remove_bg_image = "THANOSPRO.png"
    if "image" in contentType:
        with open("THANOSPRO.png", "wb") as removed_bg_file:
            removed_bg_file.write(response.content)
    else:
        await eod(rishu, f"`{response.content.decode('UTF-8')}`")
        return
    if flag and flag == "-s":
        file = tosticker(remove_bg_image, filename="THANOSPRO.webp")
    ...
    await event.client.send_file(
        event.chat_id,
        file,
        caption=f"**Background removed by** {rishu_memtion}",
        force_document=True,
        )
    await rishu.delete()
