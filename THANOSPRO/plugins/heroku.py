...
async def restart(event):
    if HEROKU_APP_NAME and HEROKU_API_KEY:
        ...
        await eor(event, f"✅ **Restarted Շђคภ๏ร-קг๏** \n**Type** `{hl}ping` **after 1 minute to check if I am working !**")
        await event.client.disconnect()


@rishu_cmd(pattern="restart$")
async def re(rishu):
    event = await eor(rishu, "Restarting Շђคภ๏ร-קг๏ ...")
    try:
        await restart(event)
...
@rishu_cmd(pattern="reload$")
async def rel(event):
    await eor(event, "Reloading Շђคภ๏ร-קг๏... Wait for few seconds...")
    await reload_THANOSPRO()


@rishu_cmd(pattern="shutdown$")
async def down(rishu):
    event = await eor(rishu, "`Turing Off Շђคภ๏ร-קг๏...`")
    await asyncio.sleep(2)
    await event.edit("**[ ⚠️ ]** \n**Շђคภ๏ร-קг๏ is now turned off. Manually turn it on to start again.**")
...
@rishu_cmd(pattern="svar(?:\s|$)([\s\S]*)")
async def sett(event):
    hel_ = event.pattern_match.group(1)
    var_ = hel_.split(" ")[0].upper()
    val_ = hel_.split(" ")[1:]
    valu = " ".join(val_)
    rishu = await eor(event, f"**Setting variable** `{var_}` **as** `{valu}`")
    if var_ == "":
        return await eod(rishu, f"**Invalid Syntax !!** \n\nTry: `{hl}svar VARIABLE_NAME variable_value`")
    elif valu == "":
        return await eod(rishu, f"**Invalid Syntax !!** \n\nTry: `{hl}svar VARIABLE_NAME variable_value`")
    if var_ not in db_config:
        return await eod(rishu, f"__There isn't any DB variable named__ `{var_}`. __Check spelling or get full list by__ `{hl}vars`")
    try:
        addgvar(var_, valu)
    except Exception as e:
        return await eod(rishu, f"**ERROR !!** \n\n`{e}`")
    await eod(rishu, f"**Variable Added Successfully!!** \n\n**• Variable:** `{var_}` \n**» Value:** `{valu}`")


@rishu_cmd(pattern="gvar(?:\s|$)([\s\S]*)")
async def gett(event):
    var_ = event.pattern_match.group(1).upper()
    rishu = await eor(event, f"**Getting variable** `{var_}`")
    if var_ == "":
        return await eod(rishu, f"**Invalid Syntax !!** \n\nTry: `{hl}gvar VARIABLE_NAME`")
    if var_ not in db_config:
        return await eod(rishu, f"__There isn't any variable named__ `{var_}`. __Check spelling or get full list by `{hl}vars`")
    try:
        sql_v = gvarstat(var_)
        os_v = os.environ.get(var_) or "None"
    except Exception as e:
        return await eod(rishu, f"**ERROR !!** \n\n`{e}`")
    await rishu.edit(f"**• OS VARIABLE:** `{var_}`\n**» OS VALUE :** `{os_v}`\n------------------\n**• DB VARIABLE:** `{var_}`\n**» DB VALUE :** `{sql_v}`\n")


@rishu_cmd(pattern="dvar(?:\s|$)([\s\S]*)")
async def dell(event):
    var_ = event.pattern_match.group(1).upper()
    rishu = await eor(event, f"**Deleting Variable** `{var_}`")
    if var_ == "":
        return await eod(rishu, f"**Invalid Syntax !!** \n\nTry: `{hl}dvar VARIABLE_NAME`")
    if var_ not in db_config:
        return await eod(rishu, f"__There isn't any variable named__ `{var_}`. Check spelling or get full list by `{hl}vars`")
    if gvarstat(var_):
        try:
            x = gvarstat(var_)
            delgvar(var_)
            await eod(rishu, f"**Deleted Variable Successfully!!** \n\n**• Variable:** `{var_}` \n**» Value:** `{x}`")
        except Exception as e:
            await eod(rishu, f"**ERROR !!** \n\n`{e}`")
    else:
        await eod(rishu, f"**No variable named** `{var_}`")
   

@rishu_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)")
async def variable(rishu):
    lg_id = Config.LOGGER_ID
    if Config.HEROKU_APP_NAME is not None:
        app = Heroku.app(Config.HEROKU_APP_NAME)
    else:
        return await eor(rishu, "**[ HEROKU ]:**\n__Please setup your__ `HEROKU_APP_NAME`")
    exe = rishu.pattern_match.group(1)
    heroku_var = app.config()
    if exe == "get":
        event = await eor(rishu, "Getting Variable Info...")
        cap = "Logger me chala jaa bsdk."
        capn = "Saved in LOGGER_ID !!"
        try:
            xvar = rishu.pattern_match.group(2).split()[0]
            variable = xvar.upper()
            if variable in db_config:
                return await eod(event, f"This is a SQL based variable. Do `{hl}gvar {variable}` to get variable info.")
            if variable in ("THANOSPRO_SESSION", "BOT_TOKEN", "HEROKU_API_KEY"):
                if Config.ABUSE == "ON":
                    await event.client.send_file(rishu.chat_id, cjb, caption=cap)
                    await event.delete()
        ...
        event = await eor(rishu, "Setting Heroku Variable...")
        xvar = rishu.pattern_match.group(2)
        if not xvar:
            return await eod(event, f"`{hl}set var <Var Name> <Value>`")
        variable = xvar.upper()
        value = rishu.pattern_match.group(3)
        if not value:
            variable = variable.split()[0]
            try:
                value = rishu.pattern_match.group(2).split()[1]
            except IndexError:
                return await eod(event, f"`{hl}set var <Var Name> <Value>`")
        if variable in db_config:
        ...
        event = await eor(rishu, "Getting info to delete Variable")
        try:
            xvar = rishu.pattern_match.group(2).split()[0]
        except IndexError:
...
@rishu_cmd(pattern="usage$")
async def dyno_usage(rishu):
    event = await eor(rishu, "`Processing...`")
    useragent = (
...
@rishu_cmd(pattern="logs$")
async def _(event):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await eor(event, f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit {rishu_grp} for help.", link_preview=False)
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        app = Heroku.app(HEROKU_APP_NAME)
    except BaseException:
        return await event.reply(f"Make Sure Your Heroku AppName & API Key are filled correct. Visit {rishu_grp} for help.", link_preview=False)
    cid = await client_id(event)
    rishu_mention = cid[2]
    rishu_data = app.get_log()
    await eor(event, rishu_data, deflink=True, linktext=f"**🗒️ Heroku Logs of 💯 lines. 🗒️**\n\n🌟 **Bot Of :**  {rishu_mention}\n\n🚀** Pasted**  ")
...
CmdHelp("power").add_command(
  ...
  "shutdown", None, "Turns off Շђคภ๏ร-קг๏. Userbot will stop working unless you manually turn it on."
).add_command(
