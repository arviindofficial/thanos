...
@rishu_cmd(pattern="kang(?:\s|$)([\s\S]*)")
async def kang(event):
    user = await event.client.get_me()
    ultronceo, rishu_USER, rishu_mention = await client_id(event)
    un = f"@{user.username}" if user.username else rishu_USER
    un_ = user.username if user.username else ultronceo
    message = await event.get_reply_message()
    THANOSPRO = gvarstat("STICKER_PACKNAME")
    photo = None
    emojibypass = False
    is_anim = False
    is_vid = False
    ...
    if message and message.media:
        if message.photo:
            rishu = await eor(event, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            ...
            if message.media.document.mime_type == "video/webm":
                ...
                for attribute in attributes:
                    if isinstance(attribute, DocumentAttributeSticker):
                        rishu = await eor(event, f"Oow! A video sticker... **[ ENCODING ]**")
                        VS = await VSticker(event, message)
                        await eor(rishu, f"`{random.choice(KANGING_STR)}`")
                        emoji = attribute.alt
                        emojibypass = True
            elif message.media.document.mime_type == "video/mp4":
                rishu = await eor(event, "Oow! A video... **[ Converting ]**")
                VS = await VSticker(event, message)
                await eor(rishu, f"`{random.choice(KANGING_STR)}`")
            is_vid = True
            photo = 1
    ...
    if photo:
        ...
        packname = f"THANOSPRO_{un_}_{pack}"
        packnick = f"{THANOSPRO}" if THANOSPRO else f"{un}'s Շђคภ๏ร-קг๏ Vol.{pack}"
        cmd = "/newpack"
        file = io.BytesIO()
        ...
        if (
            ...
            async with event.client.conversation("@Stickers") as conv:
                if not is_anim and not is_vid:
                    ...
                    while "120" in x.text:
                        ...
                        packnick = f"{THANOSPRO}" if THANOSPRO else f"{un}'s Շђคภ๏ร-קг๏ Vol.{pack}"
                        cmd = "/newpack"
                        await rishu.edit(f"`Switching to Pack {str(pack)} due to insufficient space`")
                        await conv.send_message(packname)
                        x = await conv.get_response()
                        if x.text == "Invalid set selected.":
                            ...
                            await conv.get_response()
                            await event.client.send_read_acknowledge(conv.chat_id)
                            await rishu.edit(f"**Sticker added in a Different Pack !**\nThis Pack is Newly created!\nYour pack can be found [here](t.me/addstickers/{packname})")
                            return
                    file.seek(0)
                    await conv.send_file(file, force_document=True)
                    rsp = await conv.get_response()
                    if "Sorry, the file type is invalid." in rsp.text:
                        return await eod(rishu, "`Failed to add sticker, use` @Stickers `bot to add the sticker manually.`")
                    await conv.send_message(emoji)
                    await event.client.send_read_acknowledge(conv.chat_id)
                    await conv.get_response()
                ...
                if is_anim:
                    packname = f"THANOSPRO_{un_}_{pack}_anim"
                    packnick = f"{THANOSPRO}" if THANOSPRO else f"{un}'s Շђคภ๏ร-קг๏ Vol.{pack} (Animated)"
                    cmd = "/newanimated"
                    await conv.send_message("/addsticker")
                    await conv.get_response()
                    await event.client.send_read_acknowledge(conv.chat_id)
                    ...
                    packname = f"THANOSPRO_{un_}_{pack}_vid"
                    packnick = f"{THANOSPRO}" if THANOSPRO else f"{un}'s Շђคภ๏ร-קг๏ (Video)"
                    cmd = "/newvideo"
                    await conv.send_message("/addsticker")
                    await conv.get_response()
                    await event.client.send_read_acknowledge(conv.chat_id)
                    ...
                    if "50" in xy.text:
                        pack += 1
                        return await eod(rishu, f"This Video sticker pack is already full. Use `{hl}kang {pack}` to add new stickers.")
            ...
            await rishu.edit("`Preparing a new pack....`")
            async with event.client.conversation("Stickers") as conv:
                if not is_anim and not is_vid:
                    packname = f"THANOSPRO_{un_}_{pack}"
                    packnick = f"{THANOSPRO}" if THANOSPRO else f"{un}'s Շђคภ๏ร-קг๏ Vol.{pack}"
        ...
        await tbot.send_message(
            Config.LOGGER_ID,
            f"#KANG #STICKER \n\n**A sticker has been kanged into the pack of {rishu_mention}. Click below to see the pack!**",
            buttons=[[Button.url("View Pack", f"t.me/addstickers/{packname}")]],
        )
        await eod(rishu, f"⚡** This Sticker iz [kanged](t.me/addstickers/{packname}) successfully to your pack **⚡")
...
async def get_pack_info(event):
    ...
    try:
        ...
        rishu = await eor(event, "`Fetching details of the sticker pack, please wait..`")
    except BaseException:
        await eod(event, "`This is not a sticker. Reply to a sticker.`")
        return

    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        await rishu.edit("`This is not a sticker. Reply to a sticker.`")
        return
...
async def _(event):
    ...
    rishu = await eor(event, "🥴 `Deleting sticker...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await rishu.edit("Please unblock @Stickers and try again")
            return
        await rishu.edit(response.text)


@rishu_cmd(pattern="editst(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.reply_to_msg_id:
        await eod(event, "`Reply to any user's message.`")
        return
    ...
    chat = "@Stickers"
    rishu = await eor(event, "📝 `Editing sticker emoji...`")
    if hel_ == "":
        await rishu.edit("**🤧 Nashe me hai kya lawde**")
    else:
        async with event.client.conversation(chat) as conv:
            try:
                response = conv.wait_event(
                ...
                await rishu.edit("Please unblock @Stickers and try again")
                return
            await rishu.edit(f"{response.text}")


@rishu_cmd(pattern="pkang(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if rishu == "":
        pname = f"{un}'s Շђคภ๏ร-קг๏ Pack"
    else:
        pname = rishu
    if rply and rply.media and rply.media.document.mime_type == "image/webp":
        rishu_id = rply.media.document.attributes[1].stickerset.id
        rishu_hash = rply.media.document.attributes[1].stickerset.access_hash
        got_stcr = await event.client(
            functions.messages.GetStickerSetRequest(
                stickerset=types.InputStickerSetID(id=rishu_id, access_hash=rishu_hash)
            )
        )
        stcrs = []
        ...
        try:
            create_st = await tbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=ultronceo,
                    title=pname,
                    short_name=f"rishu_{un_}_V{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
        ...
        await tbot.send_message(Config.LOGGER_ID,
                                f"#PKANG #STICKER \n\n**A sticker pack has been kanged by {rishu_mention}. Click below to see the pack!**",
...
async def sticklet(event):
    ...
    FONT_FILE = await get_font_file(event.client, "@rishuFonts")
    ...
    image_stream.name = "THANOSPRO.webp"
