...
from THANOSPRO.sql.gban_sql import is_gbanned, gbaner, ungbaner, all_gbanned
from THANOSPRO.sql.gvar_sql import gvarstat
from THANOSPRO.sql import gmute_sql as gsql
from . import *
...
@rishu_cmd(pattern="gpro(?:\s|$)([\s\S]*)")
async def _(event):
    i = 0
    sender = await event.get_sender()
    me = await event.client.get_me()
    rishu = await eor(event, "`Promoting globally...`")
    ...
    try:
        ...
        pass
    if me == user:
       k = await rishu.edit("You can't promote yourself...")
       return
    try:
        if not rank:
            rank = "ㅤ"
    except:
        return await rishu.edit("**ERROR !!**")
    if user:
        ...
        for x in telchanel:
          try:
             ...
             i += 1
             await rishu.edit(f"**Promoting User in :**  `{i}` Chats...")
          except:
             pass
    else:
        await rishu.edit(f"**Reply to a user !!**")
    await rishu.edit(
...
@rishu_cmd(pattern="gdem(?:\s|$)([\s\S]*)")
async def _(event):
    i = 0
    sender = await event.get_sender()
    me = await event.client.get_me()
    rishu = await eor(event, "`Demoting Globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await event.get_chat()
    if event.is_private:
    ...
    try:
        ...
        pass
    if me == user:
       k = await rishu.edit("You can't Demote yourself !!")
       return
    try:
        if not rank:
            rank = "ㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await event.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await rishu.edit(f"**Demoting Globally In Chats :** `{i}`")
          except:
             pass
    else:
        await rishu.edit(f"**Reply to a user !!**")
...
@rishu_cmd(pattern="gban(?:\s|$)([\s\S]*)")
async def _(event):
    rishu = await eor(event, f"`Gbanning ...`")
    reason = ""
    ...
    if event.reply_to_msg_id:
        ...
        return await eod(rishu, "**To gban a user i need a userid or reply to his/her message!!**")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == ultronceo:
        return await eod(rishu, "🥴 **Nashe me hai kya lawde ‽**")
    if str(userid) in DEVLIST:
        return await eod(rishu, "😑 **GBan my creator ?¿ Really‽**")
    ...
    if a:
        ...
        gbpic = cjb
    gmsg = f"🥴 [{name}](tg://user?id={userid}) **beta majdur ko khodna 😪 aur** {rishu_mention} **ko chodna... Kabhi sikhana nhi!! 😏**\n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
    ogmsg = f"[{name}](tg://user?id={userid}) **Is now GBanned by** {rishu_mention} **in**  `{chats}`  **Chats!! 😏**\n\n**📍 Also Added to Gban Watch!!**"
    if reason != "":
        ogmsg += f"\n**🔰 Reason :**  `{reason}`"
    if Config.ABUSE == "ON":
        ...
        await rishu.edit(ogmsg)


@rishu_cmd(pattern="ungban(?:\s|$)([\s\S]*)")
async def _(event):
    cid = await client_id(event)
    ultronceo, rishu_USER, rishu_mention = cid[0], cid[1], cid[2]
    rishu = await eor(event, "`Ungban in progress...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        ...
        return await eod(rishu, "`Reply to a user or give their userid... `")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if not is_gbanned(userid):
        return await eod(rishu, "`User is not gbanned.`")
    await rishu.edit(f"Ungbaning in client 1... \n**Unbanned in :** `{chats}`")
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
...
async def already(event):
    ...
    if len(gbanned_users) > 0:
        for user in gbanned_users:
            ...
            rishu = int(hel)
            try:
                tity = await event.client.get_entity(rishu)
                name = tity.first_name
            except ValueError:
                name = "User"
            GBANNED_LIST += f"📍 [{name}](tg://user?id={rishu}) (`{rishu}`)\n"
    else:
...
@rishu_cmd(pattern="gkick(?:\s|$)([\s\S]*)")
async def gkick(event):
    cid = await client_id(event)
    ultronceo, rishu_USER, rishu_mention = cid[0], cid[1], cid[2]
    rishu = await eor(event, f"`Kicking globally ...`")
    reply = await event.get_reply_message()
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        ...
        return await eod(rishu, "`Reply to some msg or add their id.`")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == ultronceo:
        return await eod(rishu, "**🥴 Nashe me hai kya lawde!!**")
    if str(userid) in DEVLIST:
        return await eod(rishu, "**😪 I'm not going to gkick my developer!!**")
    ...
    if Config.ABUSE == "ON":
        ...
        await rishu.edit(gkmsg)


@rishu_cmd(pattern="gmute(?:\s|$)([\s\S]*)")
async def gm(event):
    cid = await client_id(event)
    ultronceo, rishu_USER, rishu_mention = cid[0], cid[1], cid[2]
    rishu = await eor(event, "`Trying to gmute user...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        usr = event.text.split(" ", maxsplit=2)[1]
        ...
        return await eod(rishu, "**To gmute a user I'll need a userid or reply to his/her message!!**")
    name = (await event.client.get_entity(userid)).first_name
    reply = await event.get_reply_message()
    if gsql.is_gmuted(userid, "gmute"):
        return await eod(rishu, "This kid is already Gmuted.")
    if str(userid) in DEVLIST:
        return await eod(rishu, "**Sorry I'm not going to gmute them..**")
    try:
        ...
        if Config.ABUSE == "ON":
            ...
            await eor(rishu, f"**Globally Muted [{name}](tg://user?id={userid}) !!**\n\n__Kid struggling to speak__ ♪～(´ε｀ )")
    except Exception as e:
        await eod(rishu, "Error occured!\nError is " + str(e))


@rishu_cmd(pattern="ungmute(?:\s|$)([\s\S]*)")
async def endgmute(event):
    rishu = await eor(event, "`Trying to ungmute !!`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        usr = event.text.split(" ", maxsplit=2)[1]
        ...
        return await eod(rishu, "**To ungmute a user I'll need a userid or reply to his/her message!!**")
    name = (await event.client.get_entity(userid)).first_name
    reply = await event.get_reply_message()
    if not gsql.is_gmuted(userid, "gmute"):
        return await eod(rishu, "I don't remember I gmuted him/her...")
    try:
        gsql.ungmute(userid, "gmute")
        await eor(rishu, f"**Unmuted [{name}](tg://user?id={userid}) Globally !!**")
    except Exception as e:
        await eod(rishu, "Error occured!\nError is " + str(e))
