...
@rishu_cmd(pattern="autoname$")
async def _(event):
    rishu = await eor(event, "`Starting AutoName Please Wait`")
    _id, rishu_USER, _ment = await client_id(event)
    await rishu.edit(f"Auto Name has been started my Master")
    await event.client.send_message(Config.LOGGER_ID, "#AUTONAME \n\nAutoname Started!!")
    while True:
        HB = time.strftime("%d-%m-%y")
        HE = time.strftime("%H:%M")
        name = f"🕒{HE} ⚡{rishu_USER}⚡ 📅{HB}"
        logger.info(name)
        try:
            await event.client(
...
@rishu_cmd(pattern="autobio$")
async def _(event):
    rishu = await eor(event, "Starting AutoBio...")
    await rishu.edit("AutoBio Activated...")
