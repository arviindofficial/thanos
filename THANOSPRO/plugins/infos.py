...
@rishu_cmd(pattern="recognize(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    rishu = await eor(event, "recognizeing this media")
    async with event.client.conversation(chat) as conv:
        ...
        if second.text.startswith("See next message."):
            ...
            rishu = third.message.message
            await eor(event, rishu)
...
<b>⚡ <a href='https://t.me/thanospros'>From DataBase of THANOSPRO</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
@rishu_cmd(pattern="chatinfo(?:\s|$)([\s\S]*)")
async def info(event):
    rishu = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await rishu.edit(caption, parse_mode="HTML")
    except Exception as e:
        print("Exception:", e)
        await eod(rishu, "`An unexpected error has occurred.`")
    return
...
@rishu_cmd(pattern="admins(?:\s|$)([\s\S]*)")
async def _(event):
...
@rishu_cmd(pattern="id$")
async def _(event):
    rishu = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await rishu.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
