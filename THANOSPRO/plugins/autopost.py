...
from THANOSPRO.sql.autopost_sql import add_post, get_all_post, is_post, remove_post
from THANOSPRO.sql.gvar_sql import gvarstat, addgvar, delgvar

from . import *


@rishu_cmd(pattern="autopost(?:\s|$)([\s\S]*)")
async def _(event):
    if event.is_private:
        return await eod(event, "AutoPost Can Only Be Used For Channels & Groups.")
    rishu = await eor(event, "Trying to start autoposting from here...")
    ...
    if hel_ == "":
        return await eod(rishu, f"Give correct command for working of autopost. \n`{hl}autopost channel_id`")
    if str(hel_).startswith("-100"):
        kk = str(hel_).replace("-100", "")
    else:
        kk = hel_
    if not kk.isdigit():
        return await eod(rishu, "**Please Give Channel ID !!**")
    if is_post(kk , event.chat_id):
        if checker and checker == "True":
            return await rishu.edit("This channel is already in this client's autopost database.")
        else:
            addgvar(f"AUTOPOST_{str(cli_)}", "True")
            return await rishu.edit(f"**📍 Started AutoPosting from** `{hel_}` for `{cli_}`")
    add_post(kk, event.chat_id)
    addgvar(f"AUTOPOST_{str(cli_)}", "True")
    await rishu.edit(f"**📍 Started AutoPosting from** `{hel_}` for `{cli_}`")


@rishu_cmd(pattern="rmautopost(?:\s|$)([\s\S]*)")
async def _(event):
    if event.is_private:
        return await eod(event, "AutoPost Can Only Be Used For Channels.")
    rishu = await eor(event, "Removing autopost...")
    cid = await client_id(event)
    ultronceo = cid[0]
    hel_ = event.text[12:]
    cli_ = ultronceo
    checker = gvarstat(f"AUTOPOST_{str(cli_)}")
    if hel_ == "":
        return await eod(rishu, f"Give correct command for removing autopost. \n`{hl}autopost channel_id`")
    if str(hel_).startswith("-100"):
        kk = str(hel_).replace("-100", "")
    else:
        kk = hel_
    ...
    if is_post(kk, event.chat_id):
        if checker and checker == "True":
            ...
            return await eod(rishu, f"Removed `{hel_}` from `{cli_}` autopost database.")
        else:
            return await eod(rishu, f"This channel is not in `{cli_}` autopost database.")
