...
async def _(event):
    ...
    try:
        ...
        await rishu.edit(finals_)
    if os.path.exists(pic):
        os.remove(pic)


@rishu_cmd(pattern="character(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[11:]
    if query == "":
    ...
    try:
        ...
        await rishu.delete(cap_text)
    if os.path.exists(img):
        os.remove(img)


@rishu_cmd(pattern="fillers(?:\s|$)([\s\S]*)")
async def canon(event):
    rishu = event.text[9:]
    if rishu == "":
        return await eor(event, "`Give anime name to search filler episodes.`")
    nub = await eor(event, f"Searching Filler Episodes For `{rishu}`")
    hel_ = search_filler(rishu)
    if hel_ == {}:
        return await nub.edit(f"No filler found for `{rishu}`")
    ...
    if len(list_) == 1:
        ...
        return
    THANOSPRO = f"**📃 Filler Episode Lists :** \n\n"
    for i in list_:
        ...
        THANOSPRO += f"• [{i}]({paste})\n"
    await nub.edit(THANOSPRO)


@rishu_cmd(pattern="airing(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[8:]
    rishu = await eor(event, f"__Searching airing details for__ `{query}`")
    if query == "":
        return await eod(rishu, "Give anime name to seaech airing information.")
    vars_ = {"search": query}
    if query.isdigit():
    ...
    try:
        ...
        await rishu.edit(out)
    if os.path.exists(coverImg):
        os.remove(coverImg)


@rishu_cmd(pattern="aniuser(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[9:]
    rishu = await eor(event, "Searching user's Anilist Stats...")
    if query == "":
        return await rishu.edit("No user found. Give anilist username.")
    qry = {"search": query}
    result = await get_user(qry)
    if len(result) == 1:
        return await eod(rishu, result[0])
    pic, msg = result
    try:
...
CmdHelp("anime").add_command(
  ...
  "aniuser", "<anilist username>", "Searches for the Anilist Stats of the given user.", "aniuser meizrishabh"
).add_command(
