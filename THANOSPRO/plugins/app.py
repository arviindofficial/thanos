...
async def mod(event):
    ...
    if not rishu:
        if event.is_reply:
            ...
            return
    uwu = await eor(event, f"Searching Mod for  `{rishu}` ...")
    troll = await event.client.inline_query("PremiumAppBot", f"{(deEmojify(rishu))}")
