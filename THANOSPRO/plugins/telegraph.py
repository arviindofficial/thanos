...
async def _(event):
    ...
    rishu = await eor(event, "Making Telegraph Link....")
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "m":
            ...
            await rishu.edit(f"Downloaded to  `{downloaded_file_name}`  in  `{ms}`  seconds. \nMaking Telegraph Link.....")
            ...
            try:
                ...
                os.remove(downloaded_file_name)
                await rishu.edit("✓ **[File uploaded to telegraph](https://telegra.ph{})** \n✓ **Time Taken :-** `{}` secs \n✓ **By :- {}** \n✓  `https://telegra.ph{}`".format(
                        media_urls[0], (ms + ms_two), rishu_mention, media_urls[0],
                    ),
                    link_preview=True,
                )
        elif input_str == "t":
            ...
            ms = (end - start).seconds
            rishabh = f"https://telegra.ph/{response['path']}"
            await rishu.edit(f"✓ **[Pasted to telegraph]({rishabh})** \n✓ **Time Taken :-** `{ms}` secs\n✓** By :**  {rishu_mention} \n✓  `{rishabh}`", link_preview=True)
    else:
        await eod(rishu, "Reply to a message to get a permanent telegra.ph link.")
