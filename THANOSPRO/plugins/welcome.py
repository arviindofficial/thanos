...
from THANOSPRO.sql.welcome_sql import get_current_welcome, add_welcome, rm_welcome, update_welcome
...
async def _(event):
    ...
    if not gvarstat(f"WELCOME_{ultronceo}_{str(event.chat_id)[1:]}"):
        return
...
@rishu_cmd(pattern="savewelcome(?:\s|$)([\s\S]*)")
async def save_welcome(event):
    ...
    if add_welcome(event.chat_id, 0, string, msg_id) is True:
        addgvar(f"WELCOME_{ultronceo}_{str(event.chat_id)[1:]}", "TRUE")
        return await eor(event, success.format("saved"))
    rm_welcome(event.chat_id)
    delgvar(f"WELCOME_{ultronceo}_{str(event.chat_id)[1:]}")
    if add_welcome(event.chat_id, 0, string, msg_id) is True:
        addgvar(f"WELCOME_{ultronceo}_{str(event.chat_id)[1:]}", "TRUE")
...
@rishu_cmd(pattern="cleanwelcome$")
async def del_welcome(event):
    lg_id = Config.LOGGER_ID
    ultronceo, rishu_USER, rishu_mention = await client_id(event)
    if gvarstat(f"WELCOME_{ultronceo}_{str(event.chat_id)[1:]}"):
...
@rishu_cmd(pattern="welcome_note$")
