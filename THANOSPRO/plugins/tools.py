...
async def _(event):
    ...
    if not reply_message.media:
        ...
        return
    chat = "@DrWebBot"
    rishuevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(rishuevent, "`Please unblock `@DrWebBot `and try again`")
            return
        if response.text.startswith("Forward"):
            await eod(rishuevent,
...
@rishu_cmd(pattern="barcode(?:\s|$)([\s\S]*)")
async def _(event):
    rishuevent = await eor(event, "...")
    ...
    try:
        ...
        await rishuevent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await rishuevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await rishuevent.delete()


@rishu_cmd(pattern="makeqr(?: |$)([\s\S]*)")
...
async def _(event):
    ...
    if len(input_sgra) == 3:
        ...
        await rishu.edit(str(a))
    else:
        await eod(rishu, f"SYNTAX: {hl}calendar YYYY.MM.DD")


@rishu_cmd(pattern="currency(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
    input_sgra = input_str.split(" ")
    if len(input_sgra) == 3:
...
@rishu_cmd(pattern="xkcd(?:\s|$)([\s\S]*)")
async def _(event):
    rishuevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await rishuevent.edit(output_str, link_preview=True)
    else:
        await eod(rishuevent, "xkcd n.{} not found!".format(xkcd_id))

@rishu_cmd(pattern="dns(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
@rishu_cmd(pattern="unshort(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for THANOSPRO", ".currency 10 usd inr"
).add_command(
