...
async def nope(kraken):
    ...
    if not rishu:
        if kraken.is_reply:
            ...
            return
    tweeter = await event.client.inline_query("TwitterStatusBot", f"{(deEmojify(rishu))}")
...
@rishu_cmd(pattern="johhny(?:\s|$)([\s\S]*)")
async def nekobot(event):
    text = event.text[8:]
    reply_to_id = event.message
    if event.reply_to_msg_id:
...
@rishu_cmd(pattern="gandhi(?:\s|$)([\s\S]*)")
async def nekobot(event):
    text = event.text[8:]
    reply_to_id = event.message
    if event.reply_to_msg_id:
