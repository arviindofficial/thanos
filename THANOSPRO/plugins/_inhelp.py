    ...
    buttons.append(
        [
            custom.Button.inline(
               f"↞ Back {rishu_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"
            ),
            custom.Button.inline(
               f"💥 ✘ 💥", data="close"
            ),
            custom.Button.inline(
               f"{rishu_emoji} Next ↠", data=f"page({0 if page == (max_pages - 1) else page + 1})"
            ),
        ]
    )
...
if Config.BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id in auth and query == "THANOSPRO_help":
            ...
            alv_msg = gvarstat("ALIVE_MSG") or "»»» <b>Շђคภ๏รקг๏ ιѕ σиℓιиє</b> «««"
            ...
            result = builder.article(
                title="Repository",
                text=f"**⚡ ʟɛɢɛռɖaʀʏ ᴀғ Շђคภ๏รקг๏ ⚡**",
                buttons=[
                    [Button.url("📑 Repo 📑", "https://github.com/thanosuser/THANOS-PROS")],
            ...
            result = builder.article(
                "@thanospros",
                text="""**Hey! This is [Շђคภ๏ร-קг๏](https://t.me/thanospros) \nYou can know more about me from the links given below 👇**""",
                buttons=[
                    [
                        custom.Button.url("🔥 CHANNEL 🔥", "https://t.me/thanospros"),
                        custom.Button.url("⚡ GROUP ⚡", "https://t.me/THANOSPRO_chat"),
                    ],
                    [
                        custom.Button.url("✨ REPO ✨", "https://github.com/thanosuser/THANOS-PROS"),
    ...
    async def on_pm_click(event):
        ...
        if event.query.user_id in auth:
            ...
            reply_pop_up_alert = "🔰 This is Շђคภ๏ร-קг๏ PM Security to keep away unwanted retards from spamming PM !!"
    ...
    async def on_plug_in_callback_query_handler(event):
        ...
        if event.query.user_id in auth:
            ...
            await event.edit(f"**⚜️ Շђคภ๏ร-קг๏ Mêñû Prõvîdêr ìs ñôw Çlösëd ⚜️**\n\n**Bot Of :**  {rishu_mention}\n\n        [©️ Շђคภ๏ร-קг๏ ™️]({chnl_link})", buttons=veriler, link_preview=False)
        else:
            reply_pop_up_alert = "You are not authorized to use me! \n© Շђคภ๏ร-קг๏ ™"
            await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    async def Information(event):
        ...
        try:
            buttons = [
                custom.Button.inline("𓆩 " + cmd[0] + " 𓆪", data=f"commands[{commands}[{page}]]({cmd[0]})")
